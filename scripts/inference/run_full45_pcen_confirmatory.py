"""Run the P8B full-45 PCEN confirmatory Prompt V2 experiment.

This entry point prepares clean PCEN inputs from WAV audio, confirms the
required Ollama model is available on the configured HPC endpoint, and writes
raw plus parsed outputs without touching frozen baseline runs.
"""
...
CONFIG_PATH = REPO_ROOT / "configs/experiments/p8b_full45_pcen_confirmatory.yaml"
...
INPUT_DIR = REPO_ROOT / "outputs/agent_inputs/p8b_pcen_full45"
RUN_DIR = REPO_ROOT / "outputs/agent_runs/p8b_pcen_qwen3_6_full45"
...
def prepare_full45_inputs(*, overwrite_inputs: bool) -> list[dict[str, Any]]:
    """Generate full-45 clean PCEN images and return manifest rows."""
...
def main() -> None:
    ...
    if not args.skip_input_generation:
        ...
        print(f"Prepared {len(rows)} PCEN input image(s): {INPUT_DIR}")
    ...
    for clip_id in clip_ids:
        print(f"Running P8B PCEN {clip_id} with {MODEL_NAME}...")
        rows.append(
            run_one_clip(
                clip_id=clip_id,
    ...
    write_parse_summary(rows, RUN_DIR / "p8b_parse_summary.csv")
    success_count = sum(1 for row in rows if row["parse_status"] == "success")
    print(f"P8B complete: {success_count}/{len(rows)} parsed successfully.")
